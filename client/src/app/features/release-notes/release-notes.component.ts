          ...
          items: [
            {
              ...
              description: 'Blueprint1989 is fully installable as a PWA on mobile and desktop. A custom install prompt encourages installation with RemindLater and NeverAskAgain options. Device registration tracks install status per user per device.',
            ...
            },
            {
              ...
              technical: 'OfflineQueueService uses IndexedDB (idb) DB name blueprint1989Db to persist pending requests with a 24-hour TTL. Queue types: expense, payment. SyncService listens to navigator.onLine events; queued items replayed sequentially on reconnect. Failed items retained with error state for manual review.'
            }
          ]
        },
        {
