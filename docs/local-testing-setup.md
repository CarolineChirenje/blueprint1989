...
Step-by-step guide to run the Divvy PWA locally and test it on an Android phone â€” exactly as configured.
...
## Every Session â€” Start Order

You need **four things running simultaneously**, each in its own terminal.

---

### Terminal 1 â€” .NET API

```powershell
Set-Location C:\dev\6299\server\src\Divvy.Api
dotnet run
```
...
### Terminal 2 â€” Build and serve the Angular client

```powershell
# Build first (required â€” ng serve does NOT work with service workers)
Set-Location C:\dev\6299\client
npx ng build

# Then serve the output on port 80 (matches ngrok config)
http-server C:\dev\6299\client\dist\Divvy\browser -p 80 -c-1
```
...
### Terminal 3 â€” ngrok tunnel (client â†’ HTTPS)
...
Copy the `https://` URL â€” this is what you open on your Android phone.
...
### VS Code Ports Panel â€” API tunnel (port 5000 â†’ HTTPS)

1. In VS Code, click the **Ports** tab in the bottom panel
2. Click **Forward a Port** â†’ type `5000` â†’ press Enter
3. VS Code creates an `https://XXXXXXX-5000.aue.devtunnels.ms` URL
4. Right-click the port 5000 row â†’ **Port Visibility** â†’ **Public**

> The tunnel must be **Public** â€” Private tunnels require Microsoft login and won't work on your phone.
...
policy.WithOrigins(
    ...
    "https://YOUR-NEW-NGROK-URL.ngrok-free.dev",       // â† update this
...
## Testing on Android (Chrome) â€” Add to Home Screen
...
2. Navigate to the ngrok URL:
   ...
   ```
3. If ngrok shows a warning page ("You are about to visit...") â†’ tap **Visit Site**
4. Log in to the app
5. After ~30 seconds, Chrome shows an **"Add Divvy to Home Screen"** banner
   - Or tap **â‹® menu â†’ Add to Home screen**
6. Tap **Add** â€” the app icon appears on your home screen
7. Launch from the home screen â†’ app opens in standalone mode (no browser chrome)
...
3. Check **Application â†’ Manifest** â€” all 4 PNG icons should preview (192 any, 512 any, 192 maskable, 512 maskable)
4. The âŠ• install icon in the address bar confirms Chrome considers the app installable
...
| "No web page was found" for devtunnel | VS Code tunnel is Private | Ports panel â†’ right-click port 5000 â†’ Port Visibility â†’ **Public** |
...
| ngrok "endpoint already online" error | ngrok is already running â€” this is fine | Ignore the error; use existing tunnel URL from `http://localhost:4040` |
...
## Summary â€” What's Running Where
...
| ngrok tunnel | â€” | `https://YOUR-URL.ngrok-free.dev` â†’ phone |
| VS Code devtunnel | â€” | `https://XXXX-5000.aue.devtunnels.ms` â†’ phone API |
