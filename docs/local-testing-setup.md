...
Step-by-step guide to run the Blueprint1989 PWA locally and test it on an Android phone — exactly as configured.
...
```powershell
Set-Location C:\dev\Blueprint1989\server\src\Blueprint1989.Api
dotnet run
```
...
Set-Location C:\dev\Blueprint1989\client
npx ng build

# Then serve the output on port 80 (matches ngrok config)
http-server C:\dev\Blueprint1989\client\dist\Blueprint1989\browser -p 80 -c-1
```
...
> **Note:** Free ngrok URLs change every time you restart ngrok. When you get a new URL, update [Program.cs](../server/src/Blueprint1989.Api/Program.cs) CORS policy (see step below) and restart the API.
...
Update the CORS allowed origins in [server/src/Blueprint1989.Api/Program.cs](../server/src/Blueprint1989.Api/Program.cs):
...
4. Log in to the app
5. After ~30 seconds, Chrome shows an **"Add Blueprint1989 to Home Screen"** banner
...
| Symptom | Cause | Fix |
|---|---|---|
| "Target machine actively refused" on phone | http-server not running, or ngrok pointing at wrong port | Run `http-server dist/Blueprint1989/browser -p 80 -c-1`; confirm ngrok forwards port 80 |
