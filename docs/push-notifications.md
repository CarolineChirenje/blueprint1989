...
Divvy uses the **Web Push Protocol** (RFC 8030) with **VAPID authentication** to deliver real-time push notifications from the server to any browser where a user is subscribed — even when the app is not open.
...
```json
"Vapid": {
  "Subject": "mailto:dev@Divvy.local",
...
```json
"Vapid": {
  "Subject": "mailto:admin@Divvy.com",
...
1. Start the API: `dotnet run` in `server/src/Divvy.Api/`
2. Build and serve the client:
   ```powershell
   cd client
   npx ng build
   http-server dist/Divvy/browser -p 4200 -c-1
   ```
...
Open `server/src/Divvy.Api/Models/NotificationType.cs` and add the new value with the next sequential integer:
...
Open `server/src/Divvy.Api/Data/ApplicationDbContext.cs`, find `ConfigureNotificationTypeEntity()`, and add a seed row:
